...
/*Número 2*/
/*SELECT  (SELECT SUM(carga_horaria.chtar), disciplina.id_carga_horaria) FROM ((disciplina join carga_horaria on disciplina.id_carga_horaria = carga_horaria.id) join nucleo on disciplina.id_nucleo = nucleo.id) WHERE nucleo.tipo = 'disciplinas de nucleo comum'
GROUP BY disciplina.id_carga_horaria;*/
